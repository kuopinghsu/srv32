...
int main(int argc, char *argv[])
{
        ...
        printf("measure MonteCarlo.\n");
        res[3] = kernel_measureMonteCarlo(min_time, R); 
        printf("measure SparseMatMult.\n");
        ...
        /* print out results  */
        printf("Composite Score:        %8.4f\n" ,res[0]);
        printf("FFT             Mflops: %8.4f    (N=%d)\n", res[1], FFT_size);
        printf("SOR             Mflops: %8.4f    (%d x %d)\n", 		
				res[2], SOR_size, SOR_size);
        printf("MonteCarlo:     Mflops: %8.4f\n", res[3]);
        printf("Sparse matmult  Mflops: %8.4f    (N=%d, nz=%d)\n", res[4], 
					Sparse_size_M, Sparse_size_nz);
        printf("LU              Mflops: %8.4f    (M=%d, N=%d)\n", res[5],
				LU_size, LU_size);
