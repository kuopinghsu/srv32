...
class rvsim(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","rvsim")
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       self.compile_cmd = 'riscv64-unknown-elf-gcc -march={0}_zicsr \
         -static -mcmodel=medany -fvisibility=hidden -nostdlib -nostartfiles -g\
         -T '+self.pluginpath+'/env/link.ld\
         -I '+self.pluginpath+'/env/\
         -I ' + archtest_env + ' {2} -o {3} {4};\
         riscv64-unknown-elf-objdump -D {3} > {3}.objdump;\
         riscv64-unknown-elf-readelf -a {3} > {3}.readelf'
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for rvsim start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' --memsize 1716 {0}; mv dump.txt DUT-rvsim.signature'.format(elf)
          else:
            simcmd = 'echo "NO RUN"'
